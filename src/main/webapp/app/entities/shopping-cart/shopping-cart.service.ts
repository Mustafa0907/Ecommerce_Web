...
export class ShoppingCartService {
  ...
  protected convertDateFromClient(shoppingCart: IShoppingCart): IShoppingCart {
    const copy: IShoppingCart = Object.assign({}, shoppingCart, {
      createdondate: shoppingCart.createdondate && shoppingCart.createdondate.isValid() ? shoppingCart.createdondate.toJSON() : undefined
    });
    return copy;
  }
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((shoppingCart: IShoppingCart) => {
        shoppingCart.createdondate = shoppingCart.createdondate ? moment(shoppingCart.createdondate) : undefined;
      });
    }
    return res;
  }
